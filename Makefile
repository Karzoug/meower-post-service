...
POST_SERVICE_VERSION  := 0.1.0
BUILD_VERSION         ?= $(shell git symbolic-ref HEAD 2> /dev/null | cut -b 12-)-$(shell git log --pretty=format:\"%h\" -1)
BUILD_DATE            ?= $(shell date +%FT%T%z)

POST_IMAGE            := $(BASE_IMAGE_NAME)/post:$(POST_SERVICE_VERSION)

GOLANGCI_LINT_VERSION := 1.61.0

MAIN_PACKAGE_PATH     := ./cmd/
BINARY_NAME           := post_service
TEMP_DIR              := /var/tmp/meower/post
TEMP_BIN              := ${TEMP_DIR}/bin
PROJECT_PKG           := github.com/Karzoug/meower-post-service
API_FILE_NAME         := post

LDFLAGS += -s -w
LDFLAGS += -X ${PROJECT_PKG}/pkg/buildinfo.version=${BUILD_VERSION} -X ${PROJECT_PKG}/pkg/buildinfo.buildDate=${BUILD_DATE}
...
service:
	docker build \
		...
		-t $(POST_IMAGE) \
		--build-arg BUILD_REF=$(POST_SERVICE_VERSION) \
